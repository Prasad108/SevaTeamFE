...
import { Firestore, collection, doc, addDoc, updateDoc, deleteDoc, getDocs, query, where, CollectionReference, Timestamp } from '@angular/fire/firestore';
...
export interface EventVolunteerAssignment {
  ...
  adminComment: string;
  volunteerArrivalDate: Timestamp;
  pocComment: string;
...
addAssignment(assignment: EventVolunteerAssignment): Observable<EventVolunteerAssignment> {
  ...
  updateAssignment(assignment: EventVolunteerAssignment): Observable<void> {
    if (!assignment.id) {
    ...
    return from(updateDoc(assignmentDocRef, assignmentWithTimestamp)).pipe(
      map(() => console.log('Assignment updated successfully'))
    );
  }
